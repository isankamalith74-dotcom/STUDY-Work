...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://files.catbox.moe/1cac42.jpeg',
  NEWSLETTER_JID: '120363421928314589@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94726939427',
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029VbAzEINCHDyfr2jLxM3V',
  BOT_NAME: 'ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ𝗩1',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'chatuwa',
  IMAGE_PATH: 'https://files.catbox.moe/1cac42.jpeg',
  BOT_FOOTER: 'ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ',
  BUTTON_IMAGES: { ALIVE: 'https://files.catbox.moe/ditu9f.jpeg' }
};

// ---------------- MONGO SETUP ----------------

const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://manmithausa:StzuJc9Ppy4E7MG5@cluster0.0jcwzsc.mongodb.net/';
...
async function setupNewsletterHandlers(socket, sessionNumber) {
  ...
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    // SIMPLE FIX FOR IGNORE DELETE MG IN CHANNEL: Just add this one line
    ...
    try {
      ...
      const messageId = message.newsletter_server_id || message.newsletterServerId || message.key.id;
      if (!messageId) return;
...
async function setupStatusHandlers(socket, sessionNumber) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    try {
      ...
      if (autoLikeStatus === 'true' && autoViewStatus === 'true') {
...
// ---------------- Session Data Import Function ----------------

async function getSessionDataFromMongo(number) {
    try {
        await initMongo();
        const sanitized = number.replace(/[^0-9]/g, '');
        const doc = await sessionsCol.findOne({ number: sanitized });
        ...
        if (!doc) {
            console.log(`❌ No session found for ${sanitized}`);
            return null;
        }
        ...
        // Extract numbers from creds.me
        if (doc.creds && doc.creds.me) {
            // Regular number from me.id (format: "94726939427:91@s.whatsapp.net")
            if (doc.creds.me.id) {
                sessionData.regularNumber = doc.creds.me.id.split(':')[0];
            }
            
            // LID number from me.lid (format: "75227023323273:91@lid")  
            if (doc.creds.me.lid) {
                sessionData.lidNumber = doc.creds.me.lid.split(':')[0];
            }
            
            // Bot name
            if (doc.creds.me.name) {
                sessionData.name = doc.creds.me.name;
            }
        }
        
        // Device/platform info
        if (doc.creds.platform) {
            sessionData.deviceInfo.platform = doc.creds.platform;
        }
        
        /*
        console.log(`✅ Loaded session data for ${sanitized}:`, {
            regular: sessionData.regularNumber,
            lid: sessionData.lidNumber,
        ...
        */
        ...
        console.error('getSessionDataFromMongo error:', e);
        return null;
    }
}
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    const senderNumber2 = (nowsender || '').split('@')[0];
    const senderNumber = (nowsender || '').replace(/@lid|@s\.whatsapp\.net/g, '').replace(/[^0-9]/g, '');
    const sessionNumber = number.replace(/[^0-9]/g, '');
    const configOwner = senderNumber2 === config.OWNER_NUMBER.replace(/[^0-9]/g,'');

    // Import session data from MongoDB
    let sessionRegularNumber = sessionNumber;
    let sessionLidNumber = '';

    try {
        const sessionData = await getSessionDataFromMongo(sessionNumber);
        if (sessionData) {
            sessionRegularNumber = sessionData.regularNumber || sessionNumber;
            sessionLidNumber = sessionData.lidNumber || '';
        }
    } catch (error) {
        console.error('Error importing session data:', error);
    }

    // Owner detection with imported session data
    const isOwner = (
        senderNumber === sessionRegularNumber ||
        senderNumber === sessionLidNumber ||
        (sessionRegularNumber && senderNumber.includes(sessionRegularNumber)) ||
        (sessionLidNumber && senderNumber.includes(sessionLidNumber))
    );

    const isGroup = from.endsWith("@g.us");

    // FIXED: Better message body extraction
    let body = '';
    try {
        if (type === 'conversation') {
            ...
            // Handle interactive messages (like setting menu)
...
case 'setting': {
  ...
  try {
    ...
    const botName = "Ꮯ𝐇𝗔ⲧ𝐇𝒰Ꮤ𝗔";
    const prefix = currentConfig.PREFIX || config.PREFIX;

        // Create shonux object for the setting menu
      const shonux = {
      ...
      paramsJson: JSON.stringify({
        ...
        sections: [
          ...
          },
          {
            ...
            rows: [
              { title: '𝐒𝚃𝙰𝚃𝚄𝚂 𝐒𝙴𝙴𝙽 𝐎𝙽', description: '', id: `${prefix}viewstatus on` },
              { title: '𝐒𝚃𝙰𝚃𝚄𝚂 𝐒𝙴𝙴𝙽 𝐎𝙵𝙵', description: '', id: `${prefix}viewstatus off` },
            ],
          },
          {
            title: '➤ 𝐀𝚄𝚃𝙾 𝐒𝚃𝙰𝚃𝚄𝚂 𝐑𝙴𝙰𝙲𝚃',
            rows: [
              { title: '𝐒𝚃𝙰𝚃𝚄𝚂 𝐑𝙴𝙰𝙲𝚃 𝐎𝙽', description: '', id: `${prefix}reactstatus on` },
              { title: '𝐒𝚃𝙰𝚃𝚄𝚂 𝐑𝙴𝙰𝙲𝚃 𝐎𝙵𝙵', description: '', id: `${prefix}reactstatus off` },
            ],
          }, 
          {
    ...
    await socket.sendMessage(sender, {
      ...
      footer: botName,
    }, { quoted: shonux }); // Use shonux here for the quoted message
  } catch (e) {
...
case 'emojis': {
  ...
  try {
    ...
    if (!isOwner) {
      return await reply('❌ Permission denied. Only the session owner or bot owner can change status reaction emojis.');
    }
    ...
    if (!newEmojis || newEmojis.length === 0) {
      ...
      return await reply(`🎭 *Current Status Reaction Emojis:*\n\n${currentEmojis.join(' ')}\n\nUsage: \`.emojis 😀 😄 😊 🎉 ❤️\`\n\n📝 *Note:* Maximum 10 emojis allowed, separated by spaces`);
    }
    
    // 🛡️ SIMPLE CHECK: If only one argument and it contains multiple characters, show error
    if (newEmojis.length === 1 && newEmojis[0].length > 2) {
      return await reply(`❌ *Invalid format!*\n\nPlease separate emojis with spaces:\n\n❌ *Wrong:* .emojis 😀😄😊\n✅ *Correct:* .emojis 😀 😄 😊\n\nExample: \`.emojis 😀 😄 😊 🎉 ❤️\``);
    }
    
    // 🛡️ SIMPLE CHECK: Maximum 10 emojis
    if (newEmojis.length > 10) {
      return await reply(`❌ *Too many emojis!*\n\nYou provided ${newEmojis.length} emojis.\nMaximum allowed is 10 emojis.`);
    }
    
    // 🛡️ SIMPLE CHECK: Validate emojis
...
//-----------------------------------------case beake point by developer PIKO-----------------------------------------------------------//
    ...
    let botName = cfg.botName || 'ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ MINI BOT AI';
...
ඔයාගේ නම ${botName} , ඔයාගෙන් කවුරුහරි "ඔයාව හැදුවේ කවුද?" කියලා ඇහුවොත් විතරක් ඔයා කියන්නේ "මාව හැදුවේ SENUTH අයියා" කියලා. User Message: ${q}
    ...
    const { data } = await axios.post(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=AIzaSyCjME6sF9M1dEQn0t_X5p_Jc1uVxwDDojE`,
      payload,
...
END:VCARD`
    ...
    try {
        const url = `https://ᑕᕼᗩTᑌᗯᗩ-᙭ᗰᗪ-v5.onrender.com/code?number=${encodeURIComponent(number)}`;
...
> > ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ V1`
  ...
  case 'cricket':
    try {
        ...
        await socket.sendMessage(sender, {
            text: formatMessage(
                '🏏 ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ V1 MINI CEICKET NEWS🏏',
                `📢 *${title}*\n\n` +
                `🏆 *mark*: ${score}\n` +
                `🎯 *to win*: ${to_win}\n` +
                `📈 *now speed*: ${crr}\n\n` +
...
case 'alive': {
  try {
    ...
    const text = `
🤖 *${botName}* is online!
👑 *Owner*: ${config.OWNER_NAME || 'chatuwa'}
...
case 'song': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || 'ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ';
...
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '💖 ᑕᕼᗩTᑌᗯᗩ ᙭ᗰᗪ 💖';
...
`.trim();
    ...
    const defaultImg = 'https://files.catbox.moe/1cac42.jpeg';
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'CHATtUWA MINI BOT AI';
...
case 'tools': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || 'CHATUWA MINI BOT AI';
...
case 'tiktokdl': {
    try {
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'CHATUWA MINI BOT AI';
...
END:VCARD`
    ...
    try {
        ...
        // Check if newsletterMetadata function exists
        if (typeof socket.newsletterMetadata !== 'function') {
            throw new Error('newsletterMetadata function not available in this version');
        }
        ...
        if (!metadata) {
            return await socket.sendMessage(sender, {
                text: '❌ No metadata received from WhatsApp.'
            }, { quoted: shonux });
        }
...
`;
        ...
        let errorMessage = '⚠️ An unexpected error occurred while fetching channel info.';
        
        // Specific error handling
        if (err.message?.includes('newsletterMetadata function not available')) {
            errorMessage = '❌ Channel info feature is not available in this version.';
        } else if (err.message?.includes('not found') || err.message?.includes('404')) {
            errorMessage = '❌ Channel not found. Please check the link and try again.';
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of CHATU MD Mini Bot;\n' + // Title / Role
      'EMAIL;type=INTERNET:senuth2008@gmail.com\n' + // Email
      'ADR;type=WORK:;;Colombo;;Sri Lanka\n' + // Address
      'URL:https://github.com\n' + // Website
      'TEL;type=CELL;type=VOICE;waid=94741336839\n' + // WhatsApp Number
      'TEL;type=CELL;type=VOICE;waid=94765342804\n' + // Second Number (Owner)
      'END:VCARD';
...
case 'tagall': {
  try {
    ...
    let groupPP = 'https://files.catbox.moe/1cac42.jpeg';
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'CHATUWA MINI BOT AI'; // dynamic bot name
...
async function EmpirePair(number, res) {
  ...
  // ADD THIS CLEANUP CODE AT THE BEGINNING
  // If user is already connected, logout and cleanup first
  if (activeSockets.has(sanitizedNumber)) {
    console.log(`User ${sanitizedNumber} is already connected. Logging out and cleaning up...`);
    const existingSocket = activeSockets.get(sanitizedNumber);
    
    try {
      // Logout from WhatsApp
      if (existingSocket && typeof existingSocket.logout === 'function') {
        await existingSocket.logout().catch(err => {
          console.warn('Logout failed, forcing cleanup:', err);
        });
      }
      ...
      try {
        ...
        console.warn('Session file cleanup failed:', e);
      }
      
      // Remove from MongoDB sessions
      await removeSessionFromMongo(sanitizedNumber).catch(() => {});
      
      console.log(`✅ Cleanup completed for ${sanitizedNumber}, starting fresh pairing...`);
      
      // Small delay to ensure cleanup is complete
      await delay(2000);
      
    } catch (cleanupError) {
      console.error('Cleanup error during re-pairing:', cleanupError);
  ...
  // Prefill from Mongo if available (existing code)
  try {
...
router.get('/', async (req, res) => {
  ...
  // REMOVE OR MODIFY THIS CHECK - Allow re-pairing even if already connected
  // if (activeSockets.has(number.replace(/[^0-9]/g, ''))) return res.status(200).send({ status: 'already_connected', message: 'This number is already connected' });
  
  // Just log the info but allow the pairing to proceed
  const sanitized = number.replace(/[^0-9]/g, '');
  if (activeSockets.has(sanitized)) {
    console.log(`User ${sanitized} is requesting new pairing while connected. Will logout and start fresh.`);
  }
